...
class ChatRepository {
  ...
  Future<void> excluirConversa(int conversaId) async {
    await _apiClient.delete('/api/chat/conversas/$conversaId');
  }

  // Aceita compartilhar dados em conversa anônima
  Future<void> aceitarCompartilharDados(int conversaId) async {
    await _apiClient.put('/api/chat/conversas/$conversaId/compartilhar-dados', {});
  }

  Future<Map<String, dynamic>> getPerfilContato(int conversaId) async {
    final response = await _apiClient.get('/api/chat/conversas/$conversaId/contato');
    return response as Map<String, dynamic>;
  }
}
