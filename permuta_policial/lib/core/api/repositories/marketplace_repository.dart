...
class MarketplaceRepository {
  ...
  // Conta anúncios pendentes (para badge de notificação)
  Future<int> countPendentes() async {
    try {
      final response = await _apiClient.get('/api/marketplace/admin/pendentes/count');
      if (response is Map<String, dynamic> && response.containsKey('count')) {
        return (response['count'] as num).toInt();
      }
      return 0;
    } catch (e) {
      debugPrint('Erro ao contar anúncios pendentes: $e');
      return 0;
    }
  }
}