...
class DashboardProvider with ChangeNotifier {
  ...
  PermutasUnifiedMetricsResponse? _unifiedMetrics;
  ...
  PermutasUnifiedMetricsResponse? get unifiedMetrics => _unifiedMetrics;
  ...
  /// [cachedProfile] evita GET /me duplicado quando o AuthProvider já carregou na splash.
  Future<void> fetchInitialData({UserProfile? cachedProfile}) async {
    ...
    try {
      ...
      // Prioridade: intenções + matches antes de dados secundários
      ...
      // Parceiros e consultoria carregam em background
      _fetchSecondaryData();
      return;
    } catch (e) {
  ...
  Future<void> _fetchSecondaryData() async {
    try {
      ...
      final parceirosConfig = results[0] as Map<String, dynamic>;
      _exibirCardParceiros = true;
      _parceiros = parceirosConfig['parceiros'] ?? [];
      _consultoriaAdvogados = results[1] as List<ConsultoriaAdvogado>;
      notifyListeners();
    } catch (_) {}
  }
  ...
  Future<void> fetchMatches() async {
    ...
    try {
      Future<PermutasUnifiedMetricsResponse?> unifiedSafe() async {
        try {
          return await _permutasRepository.getUnifiedMetrics();
        } catch (_) {
          return null;
        }
      }
      ...
      final results = await Future.wait([
        _permutasRepository.getMatches(),
        unifiedSafe(),
      ]);
      _matches = results[0] as FullMatchResults;
      _unifiedMetrics = results[1] as PermutasUnifiedMetricsResponse?;
    } catch (e) {
  ...
  Future<bool> uploadProfilePhoto(List<int> photoBytes, {String? photoFilename}) async {
    try {
      await _policiaisRepository.uploadProfilePhoto(photoBytes, photoFilename: photoFilename);
      await fetchInitialData();
      return true;
    } catch (e) {
      if (e is ApiException) {
        ...
        _initialDataError = 'Erro ao enviar foto de perfil.';
      }
      await ErrorHandler.trackError(_analyticsService, e, endpoint: '/api/policiais/me/photo', method: 'POST');
      notifyListeners();
      return false;
    }
  }
  ...
  Future<bool> deleteProfilePhoto() async {
    try {
      await _policiaisRepository.deleteProfilePhoto();
      await fetchInitialData();
      return true;
    } catch (e) {
      if (e is ApiException) {
        ...
        _initialDataError = 'Erro ao remover foto de perfil.';
      }
      await ErrorHandler.trackError(_analyticsService, e, endpoint: '/api/policiais/me/photo', method: 'DELETE');
      notifyListeners();
      return false;
    }
  }
