...
import '../models/intencao_edit_model.dart';
...
class _GerirIntencoesModalState extends State<GerirIntencoesModal> {
  ...
  Future<void> _salvarIntencoes() async {
    ...
    for (int i = 0; i < _intencoesState.length; i++) {
      ...
      final estadoId = getItemId(intencao.selectedEstado);
      final municipioId = getItemId(intencao.selectedMunicipio);
      final unidadeId = getItemId(intencao.selectedUnidade);
      ...
      children: [
        ...
        if (intencao.tipo != null) ...[
          ...
          CustomDropdownSearch<dynamic>(
            ...
            items: _estadosCache,
            itemAsString: (e) => getItemDisplay(e, field: 'sigla'),
            onChanged: (data) {
              setState(() {
        ...
        if (intencao.tipo == 'MUNICIPIO' || intencao.tipo == 'UNIDADE') ...[
          ...
          CustomDropdownSearch<dynamic>(
            ...
            asyncItems: intencao.selectedEstado != null 
                ? (_) => dadosRepo.getMunicipiosPorEstado(getItemId(intencao.selectedEstado)!)
                : null,
            itemAsString: (m) => getItemDisplay(m, field: 'nome'),
            onChanged: (data) {
              setState(() {
          ...
          CustomDropdownSearch<dynamic>(
            ...
            asyncItems: (intencao.selectedMunicipio != null && user != null)
                ? (_) => dadosRepo.getUnidades(
                      municipioId: getItemId(intencao.selectedMunicipio)!, 
                      forcaId: user.forcaId!
                    )
                : null,
            itemAsString: (u) => getItemDisplay(u, field: 'nome'),
            onChanged: (data) => setState(() => intencao.selectedUnidade = data),
            onSuggestUnidade: intencao.selectedMunicipio != null && user != null
                ? () {
                    final municipioId = getItemId(intencao.selectedMunicipio);
