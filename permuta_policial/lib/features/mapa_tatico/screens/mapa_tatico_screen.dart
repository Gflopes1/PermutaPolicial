...
import '../widgets/mapa_tatico_manual.dart';
...
class _MapaTaticoScreenState extends State<MapaTaticoScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (mounted) {
        await MapaTaticoManual.showIfNeeded(context);
      }
    ...
    return Consumer<MapaTaticoProvider>(
      builder: (context, provider, _) {
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            actions: [
              IconButton(
                ...
                tooltip: 'Como usar o Mapa Tático',
                onPressed: () => MapaTaticoManual.show(context),
              ),
          ...
          bottomNavigationBar: _MapaTaticoBottomBar(
            selectedIndex: _bottomNavIndex,
            onSelected: (i) => setState(() => _bottomNavIndex = i),
          ),
        ...
        return Stack(
          children: [
            ...
            Positioned(
              ...
              child: Column(
                children: [
                  if (provider.privateGroups.isNotEmpty)
                    _MapControlButton(
                      icon: provider.sharingLocationEnabled ? Icons.location_on : Icons.location_off,
                      tooltip: 'Compartilhar posição com o grupo',
                      isActive: provider.sharingLocationEnabled,
                      onPressed: () =>
                          provider.setSharingLocation(!provider.sharingLocationEnabled),
                    ),
                  if (provider.privateGroups.isNotEmpty) const SizedBox(height: 8),
                  _MapControlButton(
                  ...
                  _MapControlButton(
                    ...
                    child: Text(_soundAlertEnabled ? '🔊' : '🔈', style: const TextStyle(fontSize: 16)),
                  ),
                ],
              ),
            ),
  ...
  Future<void> _showMembersSheet(MapaTaticoProvider provider) async {
    if (provider.activeGroup == null || provider.activeGroup!.isGlobal) return;
    final canManage = _canManageGroup(provider);
                  ...
                  children: [
                    ...
                    Expanded(
                      child: FutureBuilder<List<MapGroupMember>>(
                        ...
                        future: provider.getGroupMembers(),
                        builder: (context, snapshot) {
                          ...
                          return ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final canActOnMember = canManage && !isSelf && !member.isModerator;
                              ...
                              return ListTile(
                                ...
                                subtitle: Text(
                                  [
                                    member.email ?? '',
                                    if (member.isModerator) 'Moderador',
                                    if (member.isMuted) 'Mutado',
                                  ].where((e) => e.isNotEmpty).join(' • '),
                                ),
                                isThreeLine: member.isModerator || member.isMuted,
                                trailing: canActOnMember
                                    ? Wrap(
                                        spacing: 4,
                                        children: [
                                          IconButton(
                                            tooltip: 'Promover a moderador',
                                            icon: const Icon(Icons.admin_panel_settings_outlined),
                                            onPressed: () async {
                                              final ok = await showDialog<bool>(
                                                context: context,
                                                builder: (ctx) => AlertDialog(
                                                  title: const Text('Promover moderador'),
                                                  content: Text(
                                                    'Promover ${member.displayName} a moderador do grupo?',
                                                  ),
                                                  actions: [
                                                    TextButton(
                                              ...
                                              if (ok != true) return;
                                              try {
                                                await provider.promoteMember(member.userId);
                                                if (!context.mounted) return;
                                                setSheetState(() => reloadKey++);
                                                ScaffoldMessenger.of(context).showSnackBar(
                                                  AppStyles.successSnackBar('Membro promovido a moderador.'),
                                                );
                                              } catch (_) {
                                                if (!context.mounted) return;
                                                ScaffoldMessenger.of(context).showSnackBar(
                                                  AppStyles.errorSnackBar(
                                                    provider.errorMessage ?? 'Erro ao promover membro.',
                                                  ),
                                                );
                                              }
                                            },
                                          ...
                                          IconButton(
                                            ...
                                            onPressed: () async {
                                              ...
                                              try {
                                                ...
                                                setSheetState(() => reloadKey++);
                                                ScaffoldMessenger.of(context).showSnackBar(
                                                  AppStyles.successSnackBar('Membro removido do grupo.'),
                                                );
                                              } catch (_) {
                                                if (!context.mounted) return;
                                                ScaffoldMessenger.of(context).showSnackBar(
                                                  AppStyles.errorSnackBar(
                                                    provider.errorMessage ?? 'Erro ao remover membro.',
                                                  ),
                                                );
                                              }
                                            },
...
class _MapaTaticoBottomBar extends StatelessWidget {
  final int selectedIndex;
  final ValueChanged<int> onSelected;

  const _MapaTaticoBottomBar({
  ...
  static const _items = [
    ...
    (Icons.list_outlined, Icons.list, 'Lista'),
    (Icons.group_outlined, Icons.group, 'Grupo'),
  ];
