...
class MapaTaticoProvider with ChangeNotifier {
  ...
  List<MapPoint>? _cachedPointsOperational;
  List<MapPoint>? _cachedPointsLogistics;
  List<MapPoint>? _cachedPointsNational;
  PatrolLocation? _lastUiNotifiedPosition;
  ...
  List<MapPoint> get pointsOperational {
    _cachedPointsOperational ??=
        _filteredPoints([..._pointsOperational, ..._pointsShared], _filtersOperational);
    return _cachedPointsOperational!;
  }

  List<MapPoint> get pointsLogistics {
    _cachedPointsLogistics ??=
        _filteredPoints([..._pointsLogistics, ..._pointsShared], _filtersLogistics);
    return _cachedPointsLogistics!;
  }

  List<MapPoint> get pointsNational {
    _cachedPointsNational ??= _filteredPoints(_pointsNational, const MapaTaticoFilters());
    return _cachedPointsNational!;
  }
  ...
  void _upsertPoint(MapPoint point) {
    ...
    _invalidateFilteredPointCaches();
    _checkProximity();
  }
  ...
  void _invalidateFilteredPointCaches() {
    _cachedPointsOperational = null;
    _cachedPointsLogistics = null;
    _cachedPointsNational = null;
  }

  void _notifyLocationUiIfNeeded() {
    ...
    if (_lastUiNotifiedPosition == null) {
      _lastUiNotifiedPosition = pos;
      _lastUiNotifyAt = now;
      notifyListeners();
      return;
    }
    ...
    final moved = _locationService.distanceBetween(
      _lastUiNotifiedPosition!.latitude,
      _lastUiNotifiedPosition!.longitude,
      pos.latitude,
      pos.longitude,
    );
    final elapsedMs = _lastUiNotifyAt == null ? 9999 : now.difference(_lastUiNotifyAt!).inMilliseconds;
    if (moved >= 15 || elapsedMs >= 3000) {
      _lastUiNotifiedPosition = pos;
      _lastUiNotifyAt = now;
      if (_filtersOperational.maxDistanceMeters != null || _filtersLogistics.maxDistanceMeters != null) {
        _invalidateFilteredPointCaches();
      }
      notifyListeners();
    }
  ...
  Future<void> promoteMember(int userId) async {
    ...
    _errorMessage = null;
    try {
      await _repository.promoteMember(_activeGroup!.id, userId);
      await loadGroups();
    } catch (e) {
      _setError(e);
      notifyListeners();
  ...
  void _checkProximity() {
    ...
    final pos = _currentPosition!;
    final latDelta = (_alertRadiusMeters * 1.5) / 111000.0;
    final lngDelta = latDelta / math.cos(pos.latitude * math.pi / 180).abs().clamp(0.2, 1.0);
    ...
    for (final point in operationalPoints) {
      ...
      if (point.lat < pos.latitude - latDelta ||
          point.lat > pos.latitude + latDelta ||
          point.lng < pos.longitude - lngDelta ||
          point.lng > pos.longitude + lngDelta) {
        continue;
      }
  ...
  Future<MapPoint?> createPoint({
    ...
    if (mapType == 'NATIONAL') {
      ...
      _errorMessage = _activeGroup == null
          ? 'Selecione ou crie um grupo antes de adicionar pontos.'
          : 'Crie ou entre em um grupo fechado para pontos operacionais e logísticos.';
      notifyListeners();
      return null;
    }
    _errorMessage = null;
  ...
  Future<MapPoint?> deletePointPhoto(int pointId, int photoId) async {
    try {
      final updated = await _repository.deletePointPhoto(pointId, photoId);
      _upsertPoint(updated);
      notifyListeners();
      return updated;
    } catch (e) {
  ...
  Future<List<MapOccurrenceLogEntry>> getOccurrenceLog(int pointId) async {
    try {
      return await _repository.getOccurrenceLog(pointId);
    } catch (e) {
      _setError(e);
      return [];
    }
  ...
  Future<MapOccurrenceLogEntry?> addOccurrenceLogEntry(
    int pointId, {
    ...
    _errorMessage = null;
    try {
      return await _repository.createOccurrenceLogEntry(
        pointId,
        narrative: narrative,
        entryType: entryType,
        status: status,
  ...
  Future<MapSuspectProfile?> getSuspectProfile(int pointId) async {
    try {
      return await _repository.getSuspectProfile(pointId);
    } catch (e) {
      _setError(e);
      return null;
    }
  ...
  Future<MapSuspectProfile?> upsertSuspectProfile(int pointId, Map<String, dynamic> data) async {
    _errorMessage = null;
    try {
      final profile = await _repository.upsertSuspectProfile(pointId, data);
      final refreshed = await _repository.getPoint(pointId);
      _upsertPoint(refreshed);
      notifyListeners();
      return profile;
    } catch (e) {
      _setError(e);
  ...
  Future<bool> isSuspectDisclaimerAcknowledged() async {
    try {
      return await _repository.getSuspectDisclaimerAcknowledged();
    } catch (_) {
      return false;
    }
  }

  Future<bool> acknowledgeSuspectDisclaimer() async {
    try {
      await _repository.acknowledgeSuspectDisclaimer();
      return true;
    } catch (e) {
      _setError(e);
      return false;
