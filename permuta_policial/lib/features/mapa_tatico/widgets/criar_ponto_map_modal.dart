...
import '../widgets/mapa_tatico_suspect_disclaimer_dialog.dart';
import '../widgets/mapa_tatico_suspect_profile_form.dart';
...
class _CriarPontoMapModalState extends State<CriarPontoMapModal> {
  ...
  bool _privateVisibility = false;
  double? _lat;
  double? _lng;
  List<XFile> _photoFiles = [];
  final _suspectFormData = MapaTaticoSuspectProfileFormData();
  ...
  Future<void> _pickPhoto(ImageSource source) async {
    ...
    if (source == ImageSource.gallery) {
      ...
      setState(() {
        _photoFiles = [..._photoFiles, ...files].take(kMapaTaticoMaxPhotos).toList();
      });
      return;
    }
    ...
    setState(() {
      if (_photoFiles.length >= kMapaTaticoMaxPhotos) return;
      _photoFiles = [..._photoFiles, xFile];
    });
  }
  ...
  Future<void> _submit() async {
    ...
    if (_selectedType == 'suspeito') {
      if (_suspectFormData.fundamentacao.trim().length < 20) {
        setState(() {
          _errorMessage = 'Para suspeito, a fundamentação deve ter pelo menos 20 caracteres.';
        });
        return;
      }
      final provider = context.read<MapaTaticoProvider>();
      if (!await provider.isSuspectDisclaimerAcknowledged()) {
        final accepted = await showMapaTaticoSuspectDisclaimerDialog(context);
        if (!accepted) return;
        final ack = await provider.acknowledgeSuspectDisclaimer();
        if (!ack) {
          setState(() => _errorMessage = provider.errorMessage ?? 'Não foi possível registrar o aceite.');
          return;
        }
      }
    }
    ...
    try {
      ...
      if (_photoFiles.isNotEmpty) {
        if (!provider.photoUploadEnabled) {
          setState(() {
            _isLoading = false;
            _errorMessage =
                'Upload de fotos indisponível no momento. Remova as fotos ou crie o ponto sem imagem.';
          });
          return;
        }
        photos = await buildMapaTaticoPhotosFromXFiles(_photoFiles);
      }
      ...
      final point = await provider.createPoint(
        ...
        visibility: _privateVisibility ? MapPointVisibility.private : MapPointVisibility.group,
        expiresAt: expiresAt,
        photos: photos,
      );

      if (point != null && _selectedType == 'suspeito') {
        await provider.upsertSuspectProfile(point.id, _suspectFormData.toJson());
      }
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: SwitchListTile(
                      secondary: Icon(
                        Icons.lock_outline,
                        color: _privateVisibility ? Theme.of(context).colorScheme.primary : Colors.grey,
                      ),
                      title: const Text('Só eu vejo este ponto'),
                      subtitle: const Text('Ponto privado — visível apenas para você'),
                      value: _privateVisibility,
                      onChanged: (value) => setState(() => _privateVisibility = value),
                    ),
                  ),
                  if (_selectedType == 'suspeito') ...[
                    const SizedBox(height: 16),
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 16),
                      child: MapaTaticoSuspectProfileForm(data: _suspectFormData),
                    ),
                  ],
                  const SizedBox(height: 16),
                  Padding(
                    ...
                    child: Wrap(
                      ...
                      children: [
                        OutlinedButton.icon(
                          onPressed: photoUploadEnabled ? () => _pickPhoto(ImageSource.camera) : null,
                          icon: const Icon(Icons.photo_camera),
                          label: const Text('Câmera'),
                        ),
                        OutlinedButton.icon(
                          onPressed: photoUploadEnabled ? () => _pickPhoto(ImageSource.gallery) : null,
                          icon: const Icon(Icons.photo_library),
                          label: const Text('Galeria (múltiplas)'),
                        ),
                        if (_photoFiles.isNotEmpty)
                          TextButton(
                            onPressed: () => setState(() => _photoFiles = []),
                            child: const Text('Limpar fotos'),
                          ),
                      ],
                    ),
                  ),
                  if (_photoFiles.isNotEmpty)
                    Padding(
                      ...
                      child: Text(
                        '${_photoFiles.length}/$kMapaTaticoMaxPhotos foto(s) selecionada(s)',
                        style: const TextStyle(fontSize: 12, color: Colors.grey),
                      ),
                    ),
