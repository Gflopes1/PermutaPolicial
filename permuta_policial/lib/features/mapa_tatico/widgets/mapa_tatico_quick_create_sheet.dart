...
class _MapaTaticoQuickCreateSheetState extends State<MapaTaticoQuickCreateSheet> {
  ...
  Future<void> _pickPhoto(ImageSource source) async {
    if (source == ImageSource.gallery) {
      ...
      setState(() {
        _photos = [..._photos, ...files].take(kMapaTaticoMaxPhotos).toList();
      });
      return;
    }
    ...
    if (file == null) return;
    setState(() {
      if (_photos.length < kMapaTaticoMaxPhotos) {
  ...
  Future<void> _save() async {
    ...
    if (_selectedType == 'suspeito') {
      if (_suspectFormData.fundamentacao.trim().length < 20) {
        ScaffoldMessenger.of(context).showSnackBar(
          AppStyles.errorSnackBar('Fundamentação obrigatória (mín. 20 caracteres).'),
        );
        return;
      }
      if (!await provider.isSuspectDisclaimerAcknowledged()) {
        ...
        if (!ack) {
          ScaffoldMessenger.of(context).showSnackBar(
            AppStyles.errorSnackBar(provider.errorMessage ?? 'Erro ao registrar aceite.'),
          );
          return;
        }
      }
    ...
    List<http.MultipartFile>? photoFiles;
    if (_photos.isNotEmpty) {
      ...
      photoFiles = await buildMapaTaticoPhotosFromXFiles(_photos);
    }
    ...
    if (point != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        AppStyles.errorSnackBar(provider.errorMessage ?? 'Não foi possível criar o ponto.'),
          ...
          child: Column(
            ...
            children: [
              ...
              SwitchListTile(
                ...
                onChanged: _isSaving ? null : (v) => setState(() => _privateVisibility = v),
              ),
              if (_selectedType == 'suspeito') ...[
                const SizedBox(height: 8),
                MapaTaticoSuspectProfileForm(data: _suspectFormData),
              ],
              const SizedBox(height: 12),
              ...
              if (_photos.isNotEmpty)
                Padding(
                  ...
                  child: Text(
                    '${_photos.length}/$kMapaTaticoMaxPhotos foto(s)',
