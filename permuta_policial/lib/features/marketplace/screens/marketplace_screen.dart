...
import '../../verificacao/screens/verificacao_barrier_screen.dart';
...
class _MarketplaceScreenState extends State<MarketplaceScreen> 
  ...
  Future<void> _trackPageView() async {
    try {
      ...
      await analyticsService.trackPageView('/marketplace');
    } catch (e) {
      debugPrint('Erro ao rastrear page view do marketplace: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          var user = Provider.of<DashboardProvider>(
            context, 
            listen: false,
          ).userData;
          ...
          if (user == null) {
            ScaffoldMessenger.of(context).showSnackBar(
              AppStyles.errorSnackBar('Você precisa estar logado para criar anúncios'),
            );
            return;
          }

          if (userNeedsAgentVerification(user)) {
            final verified = await Navigator.of(context).push<bool>(
              MaterialPageRoute(
                builder: (_) => const VerificacaoBarrierScreen(
                  titulo: 'Verificação necessária para o Marketplace',
                  descricao:
                      'Para criar anúncios no Marketplace, sua conta precisa ser verificada como agente policial.',
                  contextoWhatsapp: 'acessar o Marketplace',
                ),
              ),
            );
            if (verified != true || !context.mounted) return;
            user = Provider.of<DashboardProvider>(context, listen: false).userData;
            if (userNeedsAgentVerification(user)) return;
          }
