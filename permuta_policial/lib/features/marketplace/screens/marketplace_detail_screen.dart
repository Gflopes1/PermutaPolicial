  ...
  Future<void> _enviarMensagem(BuildContext context) async {
    ...
    if (currentUserId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        AppStyles.errorSnackBar('Erro ao identificar usuário.'),
      );
      return;
    }

    // Não permite enviar mensagem para si mesmo
    if (currentUserId == widget.item.policialId) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      // Inicia conversa (não anônima para marketplace)
      final conversa = await chatProvider.iniciarConversa(widget.item.policialId, anonima: false);

      if (conversa != null && mounted) {
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => ChatConversaScreen(
              conversaId: conversa['id'] as int,
              outroUsuarioNome: widget.item.policialNome ?? 'Vendedor',
            ),
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          AppStyles.errorSnackBar('Erro ao iniciar conversa: ${e.toString()}'),
        );
      }
    }
  }
                        ...
                        PageView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return GestureDetector(
                              ...
                              child: Image.network(
                                resolveCdnPhotoUrl(widget.item.fotos[index]),
                                fit: BoxFit.cover,
              ...
              children: [
                ...
                Container(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Enviar Mensagem ao Vendedor'),
