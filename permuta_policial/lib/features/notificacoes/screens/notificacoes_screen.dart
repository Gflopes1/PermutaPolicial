...
class _NotificacoesScreenState extends State<NotificacoesScreen> {
  ...
  Future<void> _trackPageView() async {
    try {
      ...
      await analyticsService.trackPageView('/notificacoes');
    } catch (e) {
      debugPrint('Erro ao rastrear page view de notificacoes: $e');
    }
  }
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Erro ao carregar notificações', style: AppStyles.titleMedium),
                    AppStyles.spacingSmall,
          ...
          if (provider.notificacoes.isEmpty) {
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Icon(Icons.notifications_none, size: 80, color: AppTheme.textTertiary),
                  AppStyles.spacingSmall,
                  Text('Nenhuma notificação', style: AppStyles.titleMedium),
                  AppStyles.spacingSmall,
                  Text(
  ...
  Future<void> _handleNotificacaoTap(
    ...
    // Solicitação pendente: Aceitar/Negar ficam no card — não marcar lida nem sair.
    if (notificacao.tipo == 'SOLICITACAO_CONTATO' && !notificacao.lida) {
      return;
    }

    if (!notificacao.lida) {
      await provider.marcarComoLida(notificacao.id);
    }

    if (!mounted) return;
    await navigateFromNotificacao(context, notificacao);
  }
