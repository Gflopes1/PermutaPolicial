...
class _ForumModeracaoScreenState extends State<ForumModeracaoScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _aprovarTopico(BuildContext context, int topicoId, ForumProvider provider) async {
    ...
    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        AppStyles.successSnackBar('Tópico aprovado com sucesso!'),
      );
    } else if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        AppStyles.errorSnackBar(provider.errorMessage ?? 'Erro ao aprovar tópico'),
      );
    }
  }
  ...
  Future<void> _rejeitarTopico(BuildContext context, int topicoId, ForumProvider provider) async {
    ...
    if (confirm == true && mounted) {
      ...
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          AppStyles.successSnackBar('Tópico rejeitado.'),
        );
      } else if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          AppStyles.errorSnackBar(provider.errorMessage ?? 'Erro ao rejeitar tópico'),
        );
      }
    }
  }
  ...
  Future<void> _aprovarResposta(BuildContext context, int respostaId, ForumProvider provider) async {
    ...
    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        AppStyles.successSnackBar('Resposta aprovada com sucesso!'),
      );
    } else if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        AppStyles.errorSnackBar(provider.errorMessage ?? 'Erro ao aprovar resposta'),
      );
    }
  }
  ...
  Future<void> _rejeitarResposta(BuildContext context, int respostaId, ForumProvider provider) async {
    ...
    if (confirm == true && mounted) {
      ...
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          AppStyles.successSnackBar('Resposta rejeitada.'),
        );
      } else if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          AppStyles.errorSnackBar(provider.errorMessage ?? 'Erro ao rejeitar resposta'),
        );
      }
    }
  }
