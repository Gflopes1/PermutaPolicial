...
class _GraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final guidePaint = Paint()
      ..color = PermutaUnificadaTheme.border.withValues(alpha: 0.6)
  ...
  void _drawBackground(Canvas canvas, Size size) {
    final bg = Paint()..color = PermutaUnificadaTheme.bgDeep.withValues(alpha: 0.3);
    canvas.drawRect(Rect.fromLTWH(0, 0, size.width, size.height), bg);

    const step = 40.0;
    final dotPaint = Paint()
      ..color = PermutaUnificadaTheme.border.withValues(alpha: 0.4);
  ...
  void _drawNode(Canvas canvas, Offset pos, PermutaGraphNode node, bool selected) {
    ...
    if (isSelf) {
      ...
      _drawCenteredText(canvas, pos, node.label, 11, PermutaUnificadaTheme.textSecondary, FontWeight.w600);
    } else {
    ...
    if (!isSelf && node.kind != PermutaGraphNodeKind.overflow) {
      ...
      final tp = TextPainter(
        text: TextSpan(
          text: label,
          style: TextStyle(
            color: selected ? PermutaUnificadaTheme.textPrimary : PermutaUnificadaTheme.textSecondary,
            fontSize: 9,
  ...
  Color _colorForKind(PermutaGraphNodeKind kind) {
    switch (kind) {
      ...
      case PermutaGraphNodeKind.interessado:
        return PermutaUnificadaTheme.textSecondary;
