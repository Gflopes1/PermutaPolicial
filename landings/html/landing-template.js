...
const CSP = [
  ...
  "font-src 'self'",
  `connect-src 'self' ${SITE} https://*.tile.openstreetmap.org https://tile.openstreetmap.org`,
  "object-src 'none'",
  "base-uri 'self'",
  "form-action 'self'",
].join('; ');
...
<body data-force="${forceId}" data-force-name="${data.name}" data-tipo-permuta="${data.tipoPermuta}">
  ...
  <script defer>
    ...
    const TIPO_PERMUTA = document.body.dataset.tipoPermuta || '${data.tipoPermuta}';
    ...
    async function loadPublicStats() {
      try {
        ...
        if (stats.usuarios_verificados != null) {
          document.getElementById('userCount').textContent = stats.usuarios_verificados;
        }
        if (stats.unidades_ativas != null) {
          document.getElementById('unitCount').textContent = stats.unidades_ativas;
        }
      } catch (e) {
    ...
    function bootMaps() {
      ...
      if (!mapMain) mapMain = createMap('mapPreview', 4);
      if (!mapHero) mapHero = createMap('mapHero', 4);
      loadMapData();
    }
