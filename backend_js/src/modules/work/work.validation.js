...
// YYYY-MM-DD — não usar isoDate(): Joi converte para UTC e desloca 1 dia no fuso BR.
...
module.exports = {
  ...
  upsertDay: {
    [Segments.BODY]: Joi.object().keys({
      data: Joi.string().pattern(DATE_ONLY).required(),
  ...
  applyPreset: {
    [Segments.BODY]: Joi.object().keys({
      dates: Joi.array().items(Joi.string().pattern(DATE_ONLY)).min(1).required(),
