...
class OAuthService {
    ...
    async processOAuth({
        ...
        try {
            ...
            if (user) {
                ...
                if (referralCode) {
                    await this._attachReferralIfEligible(user.id, referralCode, user.status_verificacao);
                }
                return user;
            }
            ...
            if (user) {
                ...
                if (referralCode) {
                    await this._attachReferralIfEligible(user.id, referralCode, statusVerificacaoEmail);
                }
                return user;
            }
            ...
            if (referralCode) {
                await this._attachReferralIfEligible(newUser.id, referralCode, statusVerificacaoEmail);
            }
    ...
    async _attachReferralIfEligible(userId, referralCode, statusVerificacao) {
        if (!referralCode || !String(referralCode).trim()) return;

        const referralService = require('../referral/referral.service');
        const alreadyVerified = statusVerificacao === 'VERIFICADO';

        try {
            await referralService.createReferralOnSignup(userId, referralCode, { alreadyVerified });
        } catch (err) {
            logger.error('Erro ao criar referral OAuth', { error: err.message, userId });
        }
    }
}
