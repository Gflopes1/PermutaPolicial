...
class AdminService {
  ...
  async rebuildPermutasInteligentesGraph() {
    const { runCacheRefreshJob } = require('../permutas-inteligentes/permutas-inteligentes.service');
    console.log('[admin] Rebuild manual do grafo de permutas inteligentes solicitado');

    setImmediate(async () => {
      try {
        const result = await runCacheRefreshJob({ rebuildGraph: true, userLimit: 0 });
        console.log('[admin] Rebuild do grafo concluído:', result);
      } catch (error) {
        console.error('[admin] Rebuild do grafo falhou:', error);
      }
    });

    return {
      message:
        'Recálculo do grafo iniciado em background. Acompanhe os logs do servidor para o progresso.',
      started: true,
    };
  }

  async getActivityLogs(filters) {
    return adminRepository.getActivityLogs(filters);
  }
