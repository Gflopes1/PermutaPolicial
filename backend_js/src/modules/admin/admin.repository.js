...
class AdminRepository {
  async getEstatisticas() {
    ...
    return {
      ...
      verificacoes_ocr_pendentes: verificacoesOcrPendentes,
      ...
      const [result] = await db.execute(
        `UPDATE policiais
         SET agente_verificado = 1,
             metodo_verificacao = COALESCE(metodo_verificacao, 'manual_whatsapp'),
  ...
  async findAllPoliciais(filters = {}) {
    let query = `
      SELECT 
        ...
        COALESCE(p.agente_verificado, 0) as agente_verificado,
  ...
  async updatePolicial(policialId, updateData) {
    ...
    if (fieldsToUpdate.agente_verificado !== undefined) {
      fieldsToUpdate.agente_verificado =
        fieldsToUpdate.agente_verificado === 1 ||
        fieldsToUpdate.agente_verificado === true ||
        fieldsToUpdate.agente_verificado === '1'
          ? 1
          : 0;
    }
  ...
  async getConfiguracoes() {
    ...
    const dashboardRaw = dashboardRows.length > 0
      ? String(dashboardRows[0].valor || '').trim().toLowerCase()
      : 'antiga';
    ...
    return {
      ...
      dashboard_interface: dashboardRaw === 'nova' ? 'nova' : 'antiga',
    };
  }
  ...
  async updateConfiguracoes(updateData) {
    ...
    if (updateData['dashboard_interface'] !== undefined) {
      const valor = String(updateData['dashboard_interface']).trim().toLowerCase() === 'nova'
        ? 'nova'
        : 'antiga';
      await db.execute(
        'INSERT INTO configuracoes_gerais (chave, valor) VALUES (?, ?) ON DUPLICATE KEY UPDATE valor = ?',
        ['dashboard_interface', valor, valor]
      );
      updates.push('dashboard_interface');
    }
  ...
  async getActivityLogs({ limit = 100, offset = 0, tipo = null } = {}) {
    ...
    const pushTipo = (tipoNome, sql, sqlParams) => {
      if (tipo && tipo !== tipoNome) return;
      unions.push(`(${sql})`);
    ...
    pushTipo(
      ...
      `SELECT
         'LOGIN' AS tipo,
         ue.usuario_id AS policial_id,
         p.nome AS policial_nome,
         p.email AS policial_email,
         p.id_funcional AS id_funcional,
         ue.criado_em AS ocorrido_em,
         COALESCE(JSON_UNQUOTE(JSON_EXTRACT(ue.metadata, '$.email')), p.email, '') AS detalhe
       FROM user_events ue
       LEFT JOIN policiais p ON p.id = ue.usuario_id
       WHERE ue.evento_tipo = 'LOGIN'`,
      []
    );
    ...
    pushTipo(
      'INTENCAO_PERMUTA',
      `SELECT
         'INTENCAO_PERMUTA' AS tipo,
         i.policial_id AS policial_id,
         p.nome AS policial_nome,
         p.email AS policial_email,
         p.id_funcional AS id_funcional,
         COALESCE(i.renovado_em, i.criado_em) AS ocorrido_em,
         CONCAT(i.tipo_intencao, ' · prioridade ', i.prioridade) AS detalhe
       FROM intencoes i
       LEFT JOIN policiais p ON p.id = i.policial_id`,
      []
    );
    ...
    pushTipo(
      'INTENCAO_EDITAL',
      `SELECT
         'INTENCAO_EDITAL' AS tipo,
         ei.policial_id AS policial_id,
         p.nome AS policial_nome,
         p.email AS policial_email,
         p.id_funcional AS id_funcional,
         ei.atualizado_em AS ocorrido_em,
         COALESCE(e.titulo, CONCAT('Edital #', ei.edital_id)) AS detalhe
       FROM edital_intencoes ei
       LEFT JOIN policiais p ON p.id = ei.policial_id
       LEFT JOIN editais e ON e.id = ei.edital_id`,
      []
    );
    ...
    pushTipo(
      ...
      `SELECT
         'MENSAGEM' AS tipo,
         m.remetente_id AS policial_id,
         p.nome AS policial_nome,
         p.email AS policial_email,
         p.id_funcional AS id_funcional,
         m.criado_em AS ocorrido_em,
         LEFT(m.mensagem, 120) AS detalhe
       FROM mensagens m
       LEFT JOIN policiais p ON p.id = m.remetente_id`,
      []
    );
    ...
    const [countRows] = await db.execute(
      `SELECT COUNT(*) AS total FROM (${unionSql}) AS activity_logs`,
      params
    );
    const [rows] = await db.execute(
      `SELECT * FROM (${unionSql}) AS activity_logs
