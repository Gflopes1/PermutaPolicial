...
module.exports = {
  ...
  getCrescimentoUsuarios: handleRequest(async (req) => {
    ...
    return analyticsService.getCrescimentoUsuarios(dataInicio, dataFim, {
      granularidade: req.query.granularidade || 'dia',
      cumulativo: req.query.cumulativo === 'true' || req.query.cumulativo === '1',
      estadoId: req.query.estado_id ? parseInt(req.query.estado_id, 10) : null,
      forcaId: req.query.forca_id ? parseInt(req.query.forca_id, 10) : null,
    });
  }),

  getUsuariosPorEstado: handleRequest(async () => analyticsService.getUsuariosPorEstado()),

  getUsuariosPorForcaDetalhado: handleRequest(async () => analyticsService.getUsuariosPorForcaDetalhado()),

  getContasAtivas: handleRequest(async (req) => {
    return analyticsService.getContasAtivas(
  ...
  getMediaKitExport: handleRequest(async (req) => {
    return analyticsService.getMediaKitExport(
      req.query.data_inicio || null,
      req.query.data_fim || null
    );
  }),

  getResumoAdmin: handleRequest(async (req) => {
    return analyticsService.getResumoAdmin(
      req.query.data_inicio || null,
      req.query.data_fim || null,
      {
        granularidade: req.query.granularidade || 'dia',
