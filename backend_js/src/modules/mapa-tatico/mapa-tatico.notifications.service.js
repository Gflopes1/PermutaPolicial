...
async function notifyOperationalPointCreated(point, creatorId) {
  if (point.map_type !== 'OPERATIONAL' || point.visibility === 'PRIVATE') return;
  await notifyGroupMembers(point.group_id, creatorId, {
    ...
    mensagem: 'Novo ponto operacional no seu grupo. Toque para ver.',
  });
}
...
async function notifyPointComment(point, comment, authorId) {
  ...
  await notifyUser(point.creator_id, {
    ...
    mensagem: 'Novo comentário em um ponto do mapa. Toque para ver.',
  });
}
...
async function checkProximityAlerts(userId, groupId, lat, lng, radiusMeters = 200) {
  const points = await mapaTaticoRepository.findPointsByGroup(groupId, 'OPERATIONAL', null, userId);
  for (const point of points) {
    ...
    if (distance <= radiusMeters) {
      await notifyUser(userId, {
        ...
        mensagem: `Você está a ${Math.round(distance)} m de um ponto operacional. Toque para ver.`,
      });
      break;
    }
  }
