...
async function requirePointAccess(req, res, next) {
  ...
  if (point.visibility === 'PRIVATE' && point.creator_id !== req.user.id) {
...
function validateUploadedImage(req, res, next) {
  const { validateUploadedImages } = require('../mapa-tatico-upload.utils');
  return validateUploadedImages(req, res, next);
}

function validateUploadedImages(req, res, next) {
  const { validateUploadedImages: validateImages } = require('../mapa-tatico-upload.utils');
  return validateImages(req, res, next);
}
