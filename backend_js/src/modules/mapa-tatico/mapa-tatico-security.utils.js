...
const POINT_CREATE_RATE_LIMIT_HOURS = 1;
const POINT_CREATE_RATE_LIMIT_MAX = 20;
const SUSPECT_PROFILE_RATE_LIMIT_HOURS = 24;
const SUSPECT_PROFILE_RATE_LIMIT_MAX = 10;
...
module.exports = {
  ...
  POINT_CREATE_RATE_LIMIT_HOURS,
  POINT_CREATE_RATE_LIMIT_MAX,
  SUSPECT_PROFILE_RATE_LIMIT_HOURS,
  SUSPECT_PROFILE_RATE_LIMIT_MAX,
  safeLimit: (limit, fallback = 50, max = 100) =>
    Math.min(Math.max(parseInt(limit, 10) || fallback, 1), max),
  safeOffset: (offset) => Math.max(parseInt(offset, 10) || 0, 0),
  normalizeEmail,
