...
class NotificacoesService {
  ...
  async criarSolicitacaoContato(solicitanteId, destinatarioId, origem = null, tipoPermuta = null) {
    ...
    try {
      const pushResult = await sendPushToUser(destinatarioId, {
        title: titulo,
        body: mensagem,
        data: {
          tipo: 'SOLICITACAO_CONTATO',
          referencia_id: String(solicitanteId),
        },
      });
      if (pushResult.sent === 0) {
        logger.warn('[notificacoes] Push solicitação de contato não enviado', {
          destinatarioId,
          motivo: pushResult.skipped,
        });
      }
    } catch (pushError) {
      logger.error('Erro ao enviar push de solicitação de contato', {
  ...
  async responderSolicitacaoContato(notificacaoId, usuarioId, aceitar) {
    ...
    if (aceitar) {
      ...
      try {
        await sendPushToUser(notificacao.referencia_id, {
          title: titulo,
        ...
        logger.error('Erro ao enviar push de solicitação aceita', {
          error: pushError.message,
          solicitanteId: notificacao.referencia_id,
        });
      }
