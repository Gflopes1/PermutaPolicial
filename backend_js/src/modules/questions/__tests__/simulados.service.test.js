...
describe('SimuladosService', () => {
  ...
  describe('checkUserLimits', () => {
    ...
    it('deve rejeitar usuário free com mais de 10 questões', async () => {
      ...
      simuladosRepository.getDailyAttemptsCount = jest.fn().mockResolvedValue(0);
    ...
    it('deve verificar limite diário para usuário free', async () => {
      const simuladosRepository = require('../simulados.repository');
      simuladosRepository.getDailyAttemptsCount = jest.fn().mockResolvedValue(5);
  ...
  describe('createSimulado', () => {
    it('deve criar simulado aleatório', async () => {
      const simuladosRepository = require('../simulados.repository');
      questionsService.getRandomQuestions = jest.fn().mockResolvedValue([
        { id: 1 }, { id: 2 }, { id: 3 }
      ]);
      simuladosRepository.create = jest.fn().mockResolvedValue(1);
      simuladosRepository.addQuestions = jest.fn().mockResolvedValue(true);
      simuladosRepository.findById = jest.fn().mockResolvedValue({ id: 1 });
