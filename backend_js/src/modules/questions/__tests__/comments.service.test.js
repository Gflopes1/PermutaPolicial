...
describe('CommentsService', () => {
  ...
  describe('createComment', () => {
    it('deve criar comentário respeitando rate limit', async () => {
      commentsRepository.getRecentCommentsByUser = jest.fn().mockResolvedValue(2);
      commentsRepository.findById = jest.fn().mockResolvedValue(null);
      commentsRepository.create = jest.fn().mockResolvedValue(1);
      commentsRepository.findById = jest.fn().mockResolvedValue({ id: 1 });
    ...
    it('deve rejeitar se exceder rate limit', async () => {
      commentsRepository.getRecentCommentsByUser = jest.fn().mockResolvedValue(5);
    ...
    it('deve permitir resposta a comentário', async () => {
      commentsRepository.getRecentCommentsByUser = jest.fn().mockResolvedValue(0);
      commentsRepository.findById = jest.fn()
        .mockResolvedValueOnce({ id: 1, parent_id: null })
        .mockResolvedValueOnce({ id: 2 });
      commentsRepository.create = jest.fn().mockResolvedValue(2);
    ...
    it('deve rejeitar resposta a uma resposta', async () => {
      commentsRepository.getRecentCommentsByUser = jest.fn().mockResolvedValue(0);
      commentsRepository.findById = jest.fn().mockResolvedValue({ id: 1, parent_id: 5 });
