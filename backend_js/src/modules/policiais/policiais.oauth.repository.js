...
class PoliciaisOAuthRepository {
    ...
    /**
     * Preenche id_funcional / posto Aluno para inscrição rápida em edital.
     */
    async applyEditalQuickSignupDefaults(userId, { idFuncional = null, assignAluno = false } = {}) {
        ...
        if (!user.id_funcional && idFuncional) {
            fields.push('id_funcional = ?');
            values.push(String(idFuncional).trim());
        }

        if (assignAluno && !user.posto_graduacao_id && user.forca_id) {
            const [postos] = await db.execute(
                `SELECT pg.id
                 FROM postos_graduacoes pg
                 JOIN forcas_policiais f ON f.tipo_permuta = pg.tipo_forca
                 WHERE f.id = ? AND pg.nome = 'Aluno'
                 LIMIT 1`,
                [user.forca_id]
            );
            if (postos[0]?.id) {
                fields.push('posto_graduacao_id = ?');
                values.push(postos[0].id);
            }
        }
        ...
        values.push(userId);
        await db.execute(
            `UPDATE policiais SET ${fields.join(', ')} WHERE id = ?`,
            values
        );
