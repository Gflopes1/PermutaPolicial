...
class PoliciaisRepository {
        ...
        if (rows.length === 0) {
            ...
            try {
                ...
                if (optionalRows.length > 0) {
                    ...
                    profile.agente_verificado = Number(optionalRows[0].agente_verificado) === 1 ? 1 : 0;
                    profile.metodo_verificacao = optionalRows[0].metodo_verificacao || null;
                    profile.verificado_em = optionalRows[0].verificado_em || null;
                } else {
                    // Valores padrão
            ...
            try {
                const [alertaRows] = await db.execute(
                    `SELECT destaque_ate, COALESCE(alertas_match_ativo, 1) as alertas_match_ativo
                     FROM policiais WHERE id = ?`,
                    [id]
                );
                if (alertaRows.length > 0) {
                    profile.destaque_ate = alertaRows[0].destaque_ate;
                    profile.alertas_match_ativo = alertaRows[0].alertas_match_ativo;
                    profile.em_destaque = alertaRows[0].destaque_ate
                        ? new Date(alertaRows[0].destaque_ate) > new Date()
                        : false;
                }
            } catch (_) {
                profile.alertas_match_ativo = 1;
                profile.em_destaque = false;
            }

            try {
                const [photoRows] = await db.execute(
                    'SELECT foto_perfil FROM policiais WHERE id = ?',
                    [id]
                );
                if (photoRows.length > 0) {
                    profile.foto_perfil = photoRows[0].foto_perfil || null;
                }
            } catch (_) {
    ...
    async update(id, fieldsToUpdate) {
        ...
        if (process.env.NODE_ENV === 'development') {
            logger.log('🔧 UPDATE PERFIL:', { query, values });
        }
