...
module.exports = {
  ...
  uploadMyProfilePhoto: handleRequest(
    (req) => {
      if (!req.file?.buffer) {
        const ApiError = require('../../core/utils/ApiError');
        throw new ApiError(400, 'Nenhuma imagem foi enviada.');
      }
      return policiaisService.uploadProfilePhoto(req.user.id, req.file.buffer);
    },
    200
  ),

  deleteMyProfilePhoto: handleRequest(
    (req) => policiaisService.deleteProfilePhoto(req.user.id),
    200
  ),
};