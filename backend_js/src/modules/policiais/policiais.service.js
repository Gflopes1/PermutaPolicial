...
class PoliciaisService {
    async getProfileById(policialId) {
        ...
        try {
            ...
            profile.subscription = subscription || null;
                ...
                // Debug log
                logger.log('🔍 PoliciaisService.getProfileById - Premium Status:', {
                    policialId,
                    isPremiumFromDb,
                    hasSubscription,
            ...
            profile.subscription = null;
                ...
                logger.log('🔍 PoliciaisService.getProfileById - Fallback Premium Status:', {
                    policialId,
                    is_premium_raw: profile.is_premium,
                    finalIsPremium: profile.is_premium
                });
   ...
   async updateProfile(policialId, updateData) {
        ...
        const allowedFields = [
            ...
            'posto_graduacao_id', 'id_funcional', 'alertas_match_ativo',
        ];
        ...
        try {
            ...
            const lotacaoAlterada =
                filteredFieldsToUpdate.unidade_atual_id !== undefined ||
                filteredFieldsToUpdate.municipio_atual_id !== undefined;
            if (lotacaoAlterada) {
                const novaUnidade =
                    filteredFieldsToUpdate.unidade_atual_id !== undefined
                        ? filteredFieldsToUpdate.unidade_atual_id
                        : currentProfile.unidade_atual_id;
                const novoMunicipio =
                    filteredFieldsToUpdate.municipio_atual_id !== undefined
                        ? filteredFieldsToUpdate.municipio_atual_id
                        : currentProfile.municipio_atual_id;
                await intencoesRepository.syncLotacaoFromProfile(
                    policialId,
                    novaUnidade,
                    novoMunicipio
                );
        ...
        const graphRelevant =
            filteredFieldsToUpdate.unidade_atual_id !== undefined ||
            filteredFieldsToUpdate.municipio_atual_id !== undefined ||
            filteredFieldsToUpdate.lotacao_interestadual !== undefined ||
            filteredFieldsToUpdate.forca_id !== undefined;
        if (graphRelevant) {
            const {
                schedulePermutasCacheInvalidation,
            } = require('../permutas/permutas-cache-invalidation');
            schedulePermutasCacheInvalidation(policialId);
        }

        return { message: 'Perfil atualizado com sucesso.' };
    }

    async uploadProfilePhoto(policialId, fileBuffer) {
        const profilePhotoStorage = require('./policiais-profile-photo-storage.service');
        const currentProfile = await policiaisRepository.findProfileById(policialId);
        if (!currentProfile) {
            throw new ApiError(404, 'Perfil não encontrado.');
        }

        const fotoUrl = await profilePhotoStorage.uploadPhoto(fileBuffer);

        if (currentProfile.foto_perfil) {
            try {
                await profilePhotoStorage.deletePhoto(currentProfile.foto_perfil);
            } catch (error) {
                logger.warn('Falha ao remover foto de perfil anterior', {
                    policialId,
        ...
        const updated = await policiaisRepository.update(policialId, { foto_perfil: fotoUrl });
        if (!updated) {
            throw new ApiError(500, 'Não foi possível salvar a foto de perfil.');
        }

        return { foto_perfil: fotoUrl };
    }

    async deleteProfilePhoto(policialId) {
        const profilePhotoStorage = require('./policiais-profile-photo-storage.service');
        const currentProfile = await policiaisRepository.findProfileById(policialId);
        if (!currentProfile) {
            throw new ApiError(404, 'Perfil não encontrado.');
        ...
        if (currentProfile.foto_perfil) {
            try {
                ...
                logger.warn('Falha ao remover foto de perfil do storage', {
                    policialId,
                    message: error.message,
                });
            }
        ...
        await policiaisRepository.update(policialId, { foto_perfil: null });
