...
const { fetchUserFromDb, assertUserVerified } = require('../core/utils/policial-auth.utils');
...
function initializeSocket(server) {
  ...
  io.use(async (socket, next) => {
    try {
      ...
      const policialId = decoded.policial_id || decoded.id;
      if (!policialId) {
        return next(new Error('Token inválido'));
      }

      const policial = await fetchUserFromDb(policialId);
      const verification = assertUserVerified(policial);
      if (!verification.ok) {
        return next(new Error(verification.message));
      }

      socket.userId = policialId;
      socket.user = policial;
      next();
    } catch (error) {
